...
def open_set_identification_eval(threshold, genuine_claims, impostor_claims, all_similarities):
    ...
    for i, (label_i, similarities) in enumerate(all_similarities): #for every row (probe)
        ordered_similarities = sorted(similarities, key=lambda tup: tup[1], reverse=True)
