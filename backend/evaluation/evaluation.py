#Used to compute the similarieties among all combinations probe/templates
    ...
    for template_i, i in enumerate(0, len(template_list)): #for every row (probe)
        ...
        for template_j, j in enumerate(0, len(template_list)): #for every column (template)
            ...
            if i != j: #Do not consider main diagonal elements
                if cur_probe == cur_template: #We have to compare the identities so they must be some labels..
                    genuine_claims += 1
                else:
                    impostor_claims += 1
            row_similarities.append(0) #Must substitute 0 with the similarity algorithm (e.g. the recognition output)
